...
// The nestedFunction is using a closure. myFunction is passing down the internal trait to nestedFunction.
...
function summation(number) {
 ...
 let totalSum = sumArray.reduce(function(item, index){
    return item + index;
  },0);
...
const zooAnimals = [
  ...
  */
  function lowerCaseNames(data){
    const lowerAnimalNames = data.map(item => item.animal_name.toLowerCase());
  ...
  */
  function USApop(data){
    const animalUsPop = data.reduce(function(accumulator, item){
